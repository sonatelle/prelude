...
in
{
  perSystem =
    ...
    let
      ...
      contributionHasContent =
        c: c.packages != [ ] || c.env != [ ] || c.commands != [ ];

      namedShells = mapAttrs (name: c: {
        devshell = {
          inherit name;
    ...
    {
      options.prelude = {
        enable = mkEnableOption "Prelude development shell wiring" // {
          default = true;
        };
        ...
        base = {
          enable = mkEnableOption "Include the base contribution in merge" // {
            default = true;
          };
